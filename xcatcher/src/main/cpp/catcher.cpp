...
/* Header for class cn_vecrates_watcher_core_NativeCatcher */
...
#define LOGI(...) __android_log_print(ANDROID_LOG_ERROR,"native_catcher",__VA_ARGS__)
...
void SignalHandle(int sig, siginfo_t *si, void *context) {
    ...
    crashHandler->NotifyCrash(getpid(), gettid(), si, context);
    sleep(10); //休眠，给处理的线程足够的处理时间
...
void RegisterSignal() {
    ...
    int registerSignals[] = {SIGABRT, SIGILL, SIGBUS, SIGFPE, SIGSEGV, SIGSTKFLT, SIGSYS};
